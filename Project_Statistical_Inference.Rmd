...
```{r lybraries, warning=FALSE, echo=TRUE}
library(ggplot2)
```
...
Draw graphics on the same pane to compare them and add a Normal of the 
Distribution to see if it's similar or not. To draw the normal, we calculate the 
...
The Means distribution of a random sample of enough variables create a Normal
Distribution for the Central Limit Theorem and in this exercise we have a
demostration of this Theorem.
...
For Central limit Theorem we know that the means of this sample have a Normal
distribution.
...
Variables are Paired and supose that variances are equal
* H0: alpha1 = alpha2
* Ha: alpha1 != alpha2
...
Variables are Paired and supose that variances are equal on the three cases
* H0: alpha1 = alpha2
* Ha:alpha1 != alpha2


```{r ttesDose, echo=TRUE}
...
As happen with supp, We deny H0. So, the means are different in all cases.
